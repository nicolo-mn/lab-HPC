...
void cat_map( PGM_image* img, int k )
{
    ...
    for (i=0; i<k; i++) {
        #pragma omp parallel for collapse(2) default(none) shared(img, k, i, cur, next, tmp)
        for (y=0; y<N; y++) {
            for (x=0; x<N; x++) {
...
void cat_map_interchange( PGM_image* img, int k )
{
    ...
    /* [TODO] Which of the following loop(s) can be parallelized? */
    #pragma omp parallel for collapse(2) default(none) shared(img, k, cur, next) private(i)
    for (y=0; y<N; y++) {
        for (x=0; x<N; x++) {
