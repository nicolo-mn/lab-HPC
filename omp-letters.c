...
int local_hist[num_threads][ALPHA_SIZE];

#pragma omp parallel default(none) shared(j, text, local_hist) private(i) reduction(+:nlet)
{
    const int my_id = omp_get_thread_num();
    const int my_from = (n * my_id) / num_threads;
    const int my_to = (n * (my_id+1)) / num_threads;

    /* Reset values */
    for (i = 0; i < ALPHA_SIZE; i++) {
        local_hist[my_id][i] = 0;
    }
    /* Count occurrences */
    for (i=my_from; i<my_to; i++) {
        const char c = text[i];
        if (isalpha(c)) {
            nlet++;
