...
#pragma omp parallel default(none) shared(from,to,n,isprime,k,num_threads) reduction(+:nmarked)
    {
        int my_id = omp_get_thread_num();
        long my_from = (n * my_id / num_threads) + from;
        my_from = ((my_from + k - 1)/k)*k; /* start from the lowest multiple of p that is >= from */
        long my_to = (n * (my_id + 1) / num_threads) + from;
